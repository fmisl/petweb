...
from TF_DirectSN.utils import Dense3DSpatialTransformer
import TF_DirectSN.inputs_onlySeg as inp
...
tf.flags.DEFINE_integer('batch_size_eval', 1,
                            """Number of images to process in a batch.""")
tf.flags.DEFINE_string('data_dir1', r'E:\Direct_SN\Trainingset_onlySeg4_affine',
...
def inputs_eval(tdata_dir):
    [out1, out2, ok] = inp.eval_inputs(data_dir=tdata_dir, batch_size=FLAGS.batch_size_eval)
    return out1, out2, ok

def distorted_inputs():
    data_dir1 = os.path.join(FLAGS.data_dir1)
    [xlr, xhr, seg, seg2, seg3, seg4, seg5] = inp.distorted_inputs(data_dir=data_dir1, batch_size=FLAGS.batch_size)
    return xlr, xhr, seg, seg2, seg3, seg4, seg5
...
def random_rotation_3d(batch):
    ...
    out = batch_rot.reshape(size)
    ran_sigma = np.random.uniform(low=0.1, high=1.1)
    out = elasticdeform.deform_random_grid(out, sigma=ran_sigma, points=5, axis=(1, 2, 3))
    return out.astype(dtype=np.float32)


# deprecated
def affine_intf(tensor1,tensor2,tensor3,tensor4,tensor5,tensor6,tensor7, dim1,dim2,dim3, ranth1, ranth2, ranth3, alpha, sigma, rndx, rndy, rndz):
    ...
    # # new_xyz = [y1, x1, z1]
    t_new_vol1 = scipy.ndimage.map_coordinates(tensor1, (new_xyz), order=1)
    t_new_vol2 = scipy.ndimage.map_coordinates(tensor2, (new_xyz), order=1)
    t_new_vol3 = scipy.ndimage.map_coordinates(tensor3, (new_xyz), order=0)
    t_new_vol4 = scipy.ndimage.map_coordinates(tensor4, (new_xyz), order=0)
    t_new_vol5 = scipy.ndimage.map_coordinates(tensor5, (new_xyz), order=0)
    t_new_vol6 = scipy.ndimage.map_coordinates(tensor6, (new_xyz), order=0)
    t_new_vol7 = scipy.ndimage.map_coordinates(tensor7, (new_xyz), order=0)
    ...
    return t_new_vol1.astype(dtype=np.float32), \
           t_new_vol2.astype(dtype=np.float32), \
           t_new_vol3.astype(dtype=np.float32), \
           t_new_vol4.astype(dtype=np.float32), \
           t_new_vol5.astype(dtype=np.float32), \
           t_new_vol6.astype(dtype=np.float32), \
           t_new_vol7.astype(dtype=np.float32)
...
def dice_loss(y_true, y_pred):
    ...
    return 1 - (numerator + 1e-9) / (denominator + 1e-9)
...
def tversky_loss(y_true, y_pred):
    ...
    den = num + alpha * tf.reduce_sum(p0 * g1, (0, 1, 2, 3)) + beta * tf.reduce_sum(p1 * g0, (0, 1, 2, 3))
...
def dice_gen_dice(y_true1, y_true2, y_true3, y_true4, y_true5,
                  y_pred1, y_pred2, y_pred3, y_pred4, y_pred5):

    prediction = tf.concat([y_pred1, y_pred2, y_pred3], axis=-1)
    pred_max = tf.reduce_max(prediction)

    H, W, T, C = prediction.get_shape().as_list()[1:]
    prediction = tf.reshape(prediction, [-1, H*W*T, C])/pred_max

    one_hot = tf.concat([y_true1, y_true2, y_true3], axis=-1)
    one_hot = tf.reshape(one_hot, [-1, H*W*T, C])/pred_max

    w = tf.reduce_sum(prediction, axis=(0,1), keep_dims=True)
    w = 1/(w**2+1e-6)

    # return (w1+w2+w3)/w1*tf.losses.absolute_difference(y_true1,y_pred1) \
    # + (w1+w2+w3)/w1*tf.losses.absolute_difference(y_true2,y_pred2) \
    # + (w1+w2+w3)/w1*tf.losses.absolute_difference(y_true3,y_pred3)

    numerator = prediction * one_hot
    numerator = w * tf.reduce_sum(numerator, axis=[1])
    numerator = tf.reduce_sum(numerator, axis=-1)

    denominator = prediction + one_hot
    denominator = w * tf.reduce_sum(denominator, axis=[1])
    denominator = tf.reduce_sum(denominator, axis=-1)

    dice = 2. * (numerator + 1e-6) / (denominator + 1e-6) * pred_max

    return 1 - tf.reduce_mean(dice)

    # alpha=0.25
    # gamma=2
    # L=-one_hot*(1-alpha)*((1-prediction)*gamma)*tf.log(prediction)-\
    #   (1-one_hot)*alpha*(prediction**gamma)*tf.log(1-prediction)
    # return tf.reduce_mean(L)
    # return tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(labels=one_hot, logits=prediction, pos_weight=w))
