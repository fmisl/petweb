...
class pacs(APIView):
    ...
    def get_oneItem_dcmtk(self, Patient_ID, Study_instanceUID, Series_info, num=None):
                ...
                if study_level[1][idx_series].rstrip('\x00').find(MATCHING) is not -1 \
                    ...
                    os.popen(
                        ...
                        + study_level[3][idx_series].rstrip(
                            '\x00') + '" -od "' + self.SAVE_DIRECTORY + '" ' + self.PACS_server).read()
    ...
    def get(self, request, format=None):
        Patient_name, Patient_ID, Date_of_birth, Study_date, Modality, Study_description, Study_instanceUID, Series_info = self.find_dcmtk(date='20210526')
        print('step1')
        self.get_oneItem_dcmtk(Patient_ID, Study_instanceUID, Series_info, [0, 1])
        print('step2')
        return Response(status=status.HTTP_200_OK)